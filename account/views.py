...
class UserRegistrationView(APIView):
    def post(self,request,format=None):
        ...
        if serializer.is_valid(raise_exception=True):
            ...
            ss = UserProfileSerializer(user)
            token = get_tokens_for_user(user)
            return Response({"message":"registration successful","token":token,"user":ss.data},status=status.HTTP_201_CREATED)
...
class UserLoginView(APIView):
    def post(self,request,format=None):
        ...
        if serializer.is_valid(raise_exception=True):
            ...
            if user is not None:
                token = get_tokens_for_user(user)
                return Response({"token":token,"user":ss.data},status=status.HTTP_201_CREATED)
            else:
...
class RandomShortsId(APIView):
    def get(self,request,format=None):
        shorts_post_ids = Post.objects.filter(type="shorts").values_list('id', flat=True)
        if(shorts_post_ids):
            random_shorts_post_id = random.choice(shorts_post_ids)
            print(random_shorts_post_id)
            return Response({"id":random_shorts_post_id},status=status.HTTP_200_OK)
        else:
            return Response({'data':"no shorts to watch"},status=status.HTTP_400_BAD_REQUEST)


class GetRandomShorts(APIView):
    def get(self, request, id, format=None):
        # Retrieve the post with the provided ID
        try:
            short = Post.objects.get(pk=id)
        except Post.DoesNotExist:
            return Response({"error": "Post not found"}, status=status.HTTP_404_NOT_FOUND)

        # Serialize the post data
        serializer = PostSerializer(short)
